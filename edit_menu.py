...
class Edit():
    ...
    def find(self, *args):
        ...
        if target:
            idx = '1.0'
            while 1:
                idx = self.target.search(target, idx, nocase=1, stopindex = END)
                if not idx:
                    break
...
def main(root, text, menubar):
    ...
    # root.bind_all("<Control-c>", objEdit.copy)
    # root.bind_all("<Control-v>", objEdit.paste)
    # root.bind_all("<Control-x>", objEdit.cut)
